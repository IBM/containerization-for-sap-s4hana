...
In the following we describe how to adapt the needed environment such as
- the build LPAR 
- the NFS servers and the 
- Red Hat® OpenShift® helper and worker nodes
for running an SAP® system in Red Hat OpenShift Container Platform.
...
> **Note:** Most of the prerequisites in this chapter are prerequisites for both Python tools and Red Hat Ansible
> as long as not described different. 

## Reference SAP® System

### Setting Up the rsync Tool on the SAP Reference System

To build the images we use the `rsync` tool to copy the files locally to your build LPAR.
This tool must be installed on both your build LPAR and your SAP reference system hosts:
